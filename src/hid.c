...
static int get_manufacturer_string(IOHIDDeviceRef device, wchar_t *buf,
                                   size_t len) {
...
static int make_path(IOHIDDeviceRef device, char *buf, size_t len) {
  ...
  res = get_string_property_utf8(device, CFSTR(kIOHIDTransportKey), transport,
                                 sizeof(transport));
...
static void *read_thread(void *param) {
  ...
  IOHIDDeviceScheduleWithRunLoop(dev->device_handle, CFRunLoopGetCurrent(),
                                 dev->run_loop_mode);
    ...
    if (!strcmp(cbuf, path)) {
      ...
      if (ret == kIOReturnSuccess) {
        ...
        IOHIDManagerRegisterDeviceRemovalCallback(
            hid_mgr, hid_device_removal_callback, NULL);
...
static int set_report(hid_device *dev, IOHIDReportType type,
...
static int cond_wait(const hid_device *dev, pthread_cond_t *cond,
                     pthread_mutex_t *mutex) {
...
int HID_API_EXPORT hid_get_feature_report(hid_device *dev, unsigned char *data,
  ...
  res = IOHIDDeviceGetReport(dev->device_handle, kIOHIDReportTypeFeature,
                             data[0], /* Report ID */
                             data, &len);
  if (res == kIOReturnSuccess)
    return len;
...
void HID_API_EXPORT hid_close(hid_device *dev) {
  ...
  if (!dev->disconnected) {
    ...
    IOHIDDeviceUnscheduleFromRunLoop(dev->device_handle, dev->run_loop,
                                     dev->run_loop_mode);
    IOHIDDeviceScheduleWithRunLoop(dev->device_handle, CFRunLoopGetMain(),
                                   kCFRunLoopDefaultMode);
  }
...
static int get_device_string(hid_device *dev, const char *key, wchar_t *string,
                             size_t maxlen) {
  struct udev *udev;
...
hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
  ...
  udev_list_entry_foreach(dev_list_entry, devices) {
    ...
    /* The device pointed to by hid_dev contains information about
       ...
       it.*/
    dev = udev_device_get_parent_with_subsystem_devtype(hid_dev, "usb",
                                                        "usb_device");
    if (!dev) {
    ...
    if ((vendor_id == 0x0 && product_id == 0x0) ||
      ...
      /* Serial Number */
      cur_dev->serial_number = copy_udev_string(dev, "serial");

      /* Manufacturer and Product strings */
      cur_dev->manufacturer_string = copy_udev_string(dev, "manufacturer");
      cur_dev->product_string = copy_udev_string(dev, "product");
      ...
      intf_dev = udev_device_get_parent_with_subsystem_devtype(hid_dev, "usb",
                                                               "usb_interface");
      if (intf_dev) {
...
int HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data,
  ...
  if (bytes_read >= 0 && kernel_version < KERNEL_VERSION(2, 6, 34) &&
      dev->uses_numbered_reports) {
...
typedef BOOLEAN(__stdcall *HidD_GetPreparsedData_)(
    HANDLE handle, HIDP_PREPARSED_DATA **preparsed_data);
typedef BOOLEAN(__stdcall *HidD_FreePreparsedData_)(
    HIDP_PREPARSED_DATA *preparsed_data);
typedef BOOLEAN(__stdcall *HidP_GetCaps_)(HIDP_PREPARSED_DATA *preparsed_data,
                                          HIDP_CAPS *caps);
...
static hid_device *new_hid_device() {
  ...
  dev->ol.hEvent =
      CreateEvent(NULL, FALSE, FALSE /*inital state f=nonsignaled*/, NULL);
...
static void register_error(hid_device *device, const char *op) {
  ...
  FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
                 ...
                 NULL, GetLastError(),
                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPWSTR)&msg,
                 0 /*sz*/, NULL);
...
static HANDLE open_device(const char *path) {
  ...
  handle = CreateFileA(path, GENERIC_WRITE | GENERIC_READ, 0, /*share mode*/
                       NULL, OPEN_EXISTING,
  ...
  if (handle == INVALID_HANDLE_VALUE) {
    /* Couldn't open the device. Some devices must be opened
       ...
       so try it here. */
    handle = CreateFileA(path, GENERIC_WRITE | GENERIC_READ,
...
hid_enumerate(unsigned short vendor_id, unsigned short product_id) {
  ...
  for (;;) {
    ...
    res =
        SetupDiEnumDeviceInterfaces(device_info_set, NULL, &InterfaceClassGuid,
    ...
    res = SetupDiGetDeviceInterfaceDetailA(
        device_info_set, &device_interface_data, device_interface_detail_data,
...
HID_API_EXPORT hid_device *HID_API_CALL hid_open(unsigned short vendor_id,
                                                 unsigned short product_id,
                                                 wchar_t *serial_number) {
  // TODO: Merge this functions with the Linux version. This function should be
  // platform independent.
  struct hid_device_info *devs, *cur_dev;
  const char *path_to_open = NULL;
  hid_device *handle = NULL;
...
int HID_API_EXPORT HID_API_CALL hid_read_timeout(hid_device *dev,
  ...
  if (!dev->read_pending) {
    ...
    ResetEvent(ev);
    res = ReadFile(dev->device_handle, dev->read_buf, dev->input_report_length,
                   &bytes_read, &dev->ol);
...
int HID_API_EXPORT HID_API_CALL hid_get_feature_report(hid_device *dev,
  ...
  res = DeviceIoControl(dev->device_handle, IOCTL_HID_GET_FEATURE, data, length,
